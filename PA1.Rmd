...
and find that the daily mean is `r mean_steps` and the daily median is `r median_steps`.
...
The interval with the maximum average daily steps is `r max_interval`.
...
We found that `r num_NAs` rows contain missing values.
...
We see that the mean of daily steps has stayed at `r imean_steps` (from `r mean_steps`) but the median has shifted to `r imedian_steps` (from `r median_steps`).
